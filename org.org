...
- [ ] Implement RoomZone (collect_metrics, identify_waste, act)
- [ ] Implement FloorZone (aggregate room metrics upward)
- [ ] Implement BuildingZone (aggregate floor metrics upward)
- [ ] Create simulation engine (realistic sensor data with time-based drift)
- [ ] Inject waste scenarios for demo (empty room heating, open window, standby appliances)
- [ ] Wire WebSocket /ws endpoint with live MetricsUpdate stream
...
- [ ] Implement Room/Floor/Building EnergyZone hierarchy
- [ ] Build simulation engine with waste scenarios
- [ ] Wire WS /ws endpoint with live metrics stream
