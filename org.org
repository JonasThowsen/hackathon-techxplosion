...
** Node-edge analysis
Compute where heat or air quality goes.
Things we know:
- Heat production and the power cost
- Room temperature
Things we dont know:
- How conductive walls are
- If windows or doors are open
* Tasks
...
** Friday evening
*** Dany (Frontend)
- [X] Create TypeScript types matching the contract (BuildingLayout, MetricsUpdate)
- [X] Build FloorGrid canvas component that takes a floor + metrics and draws polygons
- [X] Hardcode mock BuildingLayout in frontend to test rendering without backend
- [X] Color rooms with a basic heatmap color function (blue cold → red hot)
- [X] Verify rooms draw correctly, walls visible, no overlaps
*** Jonas (Backend)
...
** Saturday morning
*** Dany (Frontend)
- [X] Replace mock data with =fetch("/building")= call on mount
- [X] Connect to =WS /ws=, parse MetricsUpdate messages
- [X] Merge incoming metrics into stored geometry, re-render canvas on each tick
- [X] Add floor selector (tabs or buttons)
*** Jonas (Backend)
- [X] Add CORS config so frontend can connect from different port
...
** Saturday afternoon
*** Jonas (Backend)
- [ ] Define room baselines / setpoints (target temp per room type, heating thresholds)
- [ ] Building-level waste detection in =BuildingZone.identify_waste()= (compare metrics vs baselines)
- [ ] Building-level actions in =BuildingZone.act()= (generate corrective actions from waste patterns)
- [ ] Include waste patterns + actions in =MetricsUpdate= for the frontend
- [ ] Simulation: rooms should drift toward setpoints when actions fire (close the feedback loop)
